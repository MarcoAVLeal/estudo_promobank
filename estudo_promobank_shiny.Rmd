...
# 
# 
# class_seq <- seq(0,1000,50)
# 
# df_matrix_0  <- df_scores %>% dplyr::select(c("SCORE_SCR","SCORE_SPC")) 
# df_matrix_0$SPC_CLASS   <- arules::discretize(df_matrix_0$SCORE_SPC,    method = "fixed",labels = NULL, breaks = class_seq)
# 
# 
# 
# 
# df_matrix_0$SCR_CLASS <- arules::discretize(df_matrix_0$SCORE_SCR,  method = "fixed",labels = NULL, breaks = class_seq)
...
# df_score$PREDICT <- 1-(df_score$SCORE/1000)
# df_score$PREDICT_INV <- (df_score$SCORE/1000)
df_bons <- df$Valor.Benefício
df_maus <- df$RENDAPRESUMIDA

seq_score <- eq(0,15500,1)
...
p1 <- ggplot(data = df_melt)+
  ...
  scale_x_continuous(breaks = seq(0,1*15500,0.05*15500))+
  scale_y_continuous(breaks = seq(0,1,0.1))+
...
ks <- ks.test(x = df$Valor.Benefício,y = df$RENDAPRESUMIDA)

 renderUI({
       
        p(ks$p.value,br(), " Teste Kolmogorov-Smirnov",style="color:#273658;text-align:center;font-weight:bold;font-size:30px")
        
      })

```
Aqui sera apresentado o teste de kS.
